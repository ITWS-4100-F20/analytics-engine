...
from simulation.support.modelHelper import numeric, keyed_numeric
...
class Comptroller(object):
    ...
    def Bid(self, passenger:dict, timeleft:int):
        ...
        for i in comp_types.keys():
            ...
            row["comp_amount"] = self.compMod.predict(pdata)[0]
            pdata = numpy.array([[row[i] for i in self.keys if i != "pass-result" and i != "comp-target"]]).reshape(1,-1)
            row["comp-result"] = self.passMod.predict_proba(pdata)[0][0]
            comp_types[i]["value"] = row["comp_amount"]
            comp_types[i]["pass"] = row["comp-result"]
            value = 1 - comp_types[i]["pass"] * comp_types[i]["value"] * (5 if i == 0 else 1)
            if value < choice:
                key = i
                choice = value
